...
- **Több LLM adapter** — `LLM_PROVIDER=gemini` (ingyenes tier, `GEMINI_API_KEY`, jelenlegi tesztüzemmód) vagy `LLM_PROVIDER=anthropic` (fizetős, opcionális, egyelőre nem használt); mindkettő token-/költségadata (`provider`, `model`, tokenek, `cost_usd`) a `llm_usage` táblába kerül. A Gemini adaptert egy reaktív fallback (`ProviderFallbackLlmClient`) csomagolja: kvóta-/403-/szolgáltatáshiba esetén automatikusan No-LLM módra vált, a pipeline nem áll le. Az Anthropic-ág emellett proaktív havi költségplafonnal is véd (`LLM_MONTHLY_BUDGET_USD`, alapértelmezés: 5 USD);
...
packages/llm      — LLM kliens-absztrakció (Gemini free-tier + Anthropic SDK, provider-fallback)
