...
- `magyarsportonline-ingest-scheduler` targets `https://magyarsportonline.hu`, has the same rotated `CRON_SECRET`, and runs every minute. Scheduler version `2c8877b4-7b6a-418e-88d8-768517fa896f` is active. Authenticated production run `34740451358` reached the custom domain, returned HTTP 200 from both `/api/internal/cron/dispatch-ingest` and `/api/internal/jobs/process`, processed three newly ingested jobs successfully, and drained the pending queue.
...
- The `.hu` registry delegates `magyarsportonline.hu` to `ethan.ns.cloudflare.com` and `tori.ns.cloudflare.com`, and the Cloudflare dashboard reports the zone as Active. `https://magyarsportonline.hu/`, `https://www.magyarsportonline.hu/`, and the Workers.dev rollback origin all return HTTP 200 with Cloudflare and OpenNext response headers.
