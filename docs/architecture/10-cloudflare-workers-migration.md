...
- `magyarsportonline-ingest-scheduler` targets `https://magyarsportonline.hu`, has the same rotated `CRON_SECRET`, and runs every minute. Authenticated smoke tests for `/api/internal/cron/dispatch-ingest` and `/api/internal/jobs/process` both return HTTP 200.
