...
| **Folyamat** | 1) **Generálás**: Cloudflare Workers AI prompt, ami kizárólag a megadott `Fact` JSON-t használhatja fel, explicit instrukcióval ("ne tegyél hozzá semmit, ami nincs a tények között") → cím, lead, törzs, `change_summary_hu` → 2) **Önellenőrzés (NLI/entailment)**: egy rövid, 512 output tokenre korlátozott Cloudflare-hívás minden generált állítást visszavet a `Fact`-készletre. Inkonzisztencia esetén nincs vak újragenerálás: az alacsony pontszám megmarad, a Publish Gate fail-closed review-ba irányítja. |
...
| **Modellválasztás** | Egyetlen production provider és modell: Cloudflare Workers AI `@cf/meta/llama-3.3-70b-instruct-fp8-fast`. A költséget prompt-, output-token- és hívásszám-korlátok fogják meg; nincs provider-fallback. |
