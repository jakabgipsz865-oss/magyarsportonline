...
## 13. Story-matching: a determinisztikus fingerprint téves pozitív összevonáshoz vezetett → pontozott, többfeltételes motor

- **Kérdés:** A 2026-07-29-i bizonyító riport (11. és 12. tétel után) feltárta: a talált EGYETLEN "kétforrásos" Story valójában TÉVES POZITÍV összevonás volt — 16, teljesen különböző eseményről szóló cikk (darts, golf, krikett, lóverseny, F1, tenisz, box, egy UEFA/FIFA-hír, egy Rangers-menedzser-hír, egy BBC "Ki vagyok én?" kvíz) olvadt össze egyetlen Storyba, mert mindegyik cikk szövege valahol tartalmazta a "Premier League" szót (feltehetően a Sky Sports-cikkek lábléc-jellegű, promóciós CTA-bekezdéseiben, pl. "Watch Premier League and more with NOW"), és mind ugyanazon a napon érkeztek — a régi fingerprint (egyetlen "elsődleges entitás", teljes cím+törzs szövegen substring-kereséssel, típus-prioritás alapján kiválasztva: csapat > játékos > verseny > liga > helyszín) ezt tévesen "ugyanaz az esemény"-ként azonosította.
- **Jelenlegi döntés:** A felhasználó explicit, részletes specifikációja alapján (2026-07-29) a determinisztikus egyetlen-entitás fingerprint LECSERÉLVE egy pontozott, többfeltételes Story-matching motorra (`packages/agents/src/deduplication/story-match.ts` + `entity-mentions.ts` + `sport.ts`). Kötelező szabályok, mind implementálva és tesztelve: (1) verseny/liga/helyszín-típusú entitás önmagában SOHA nem elég automatikus összevonáshoz; (2) automatikus összevonáshoz legalább egy SPECIFIKUS (csapat/játékos) közös entitás kell; (3) a cím és a lead (subtitle vagy az első, nem-boilerplate bekezdés) nagyobb súlyt kap, mint a teljes cikktörzs — az új motor a specifikus-entitás-egyezést KIZÁRÓLAG cím/lead alapján vizsgálja, a teljes törzset egyáltalán nem szkenneli; (4) ismert boilerplate/CTA-mondatminták (pl. "Stream ... with NOW", "Watch ... live on Sky Sports") ki vannak zárva a lead-származtatásból; (5) különböző sportágak cikkei SOHA nem vonhatók össze — a sportág az URL saját elérési útjából derül ki (`sport.ts`, pl. `skysports.com/{sport}/...`, `bbc.co.uk/sport/{sport}/...`), determinisztikusan, `null`-ra esve ismeretlen esetben (sosem blokkol); (6) bizonytalan eset (van specifikus közös entitás, de a pontszám nem éri el az automatikus küszöböt) NEM vonódik össze — saját Story jön létre, és `needs_review`/`pending` állapotú sor kerül a `story_match_decisions` táblába kézi felülvizsgálatra; (7) MINDEN döntés (auto_merge/needs_review/auto_new_story) perzisztálva van: match score, egyező entitások, eltérő entitások, sportág-eltérés, döntési indoklás (magyarul), automatikus vagy kézi jelleg.
- **Bizonyíték:** (a) Unit tesztek (`story-match.test.ts`, `entity-mentions.test.ts`, `sport.test.ts`) — köztük egy explicit REGRESSZIÓS teszt, ami a valós 16-cikkes hibát rekonstruálja (ugyanazok a cím/CTA-szövegek) és igazolja, hogy az új motor NEM vonja össze őket. (b) Valós Postgres elleni E2E-teszt (scratch script, futtatva és törölve, nem commitolva): 1. a rekonstruált 16-cikkes (itt 4 reprezentatív cikk) jelenet — mind a 4 külön Storyba került, nulla összevonás; 2. két cikk, ami VALÓBAN ugyanarról a mérkőzésről szól (közös "Liverpool" csapat-entitás, ugyanaz a nap) — helyesen automatikusan összevonva (`MATCH`, közös Story); 3. egy bizonytalan eset (közös "Arsenal" csapat-entitás, de eltérő nap, nincs kiegészítő jel) — helyesen `AMBIGUOUS`/`needs_review`-ba került, SAJÁT Storyt kapott, NEM olvadt össze a jelölt Storyval, és a döntés perzisztálva lett a `story_match_decisions` táblában a helyes `candidateStoryId`/`resultingStoryId` párral.
- **Nyitott kockázat:** A jelenlegi entitás-táblában (`packages/db/src/seed.ts`) ZÉRÓ `player`-típusú entitás van seedelve — csak csapatok és versenyek. Emiatt a "specifikus entitás" szabály a gyakorlatban ma kizárólag csapat-alapú egyezésre tud támaszkodni; az edző/játékos-alapú egyezés (a szabály 2. pontjának "ugyanaz a játékos vagy edző" fele) csak akkor fog ténylegesen működni, ha a jövőben játékos-entitások is bekerülnek a táblába. Ez nem blokkolja a jelen javítást (a csapat-alapú védelem már önmagában megszünteti a bizonyított hibát), de korlátozza, mennyi valódi kétforrásos Story lesz felismerhető, amíg a játékos-taxonómia nem bővül.
- **Következő teendő:** Játékos-entitások seedelése (külön feladat, nem blokkoló) a "ugyanaz a játékos/edző" szabály tényleges kihasználásához. A bizonyító riport (`credibility-proof-report.yml`) kibővítve: `matchDecisions` szakasz mutatja az `auto_merge`/`needs_review`/`auto_new_story` bontást, minden `auto_merge` és `needs_review` döntés teljes bizonyítékával (match score, egyező/eltérő entitások, indoklás) — a precision/recall szám explicit NEM kerül kitalálásra a végpontban, csak azután számolható, hogy egy ember ténylegesen elolvasta és megerősítette az `auto_merge` döntéseket.
- **Felelős/státusz:** Implementáció KÉSZ, tesztelve (unit + valós Postgres E2E), a régi hibát okozó fingerprint-mechanizmus (egyetlen, teljes-szövegen substring-kereséssel talált entitás) teljesen eltávolítva a döntési útból. Hátralévő: az új motorral gyűjtött valós production adaton a bizonyító riport tényleges lefuttatása és a precision/recall kézi megerősítése.
