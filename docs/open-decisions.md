...
- **Következő teendő:** A job-alapú pipeline megépítése (külön PR-ok: `pipeline_jobs` tábla+repo — KÉSZ, PR #45; majd a queueing dispatcher+worker route+ingest átkötés — folyamatban), majd csak ezután a valós BBC+Sky ingest újraindítása és a bizonyító riport tényleges, ≥1 valódi dual-source Storyt tartalmazó lefuttatása.
- **Felelős/státusz:** Döntés lezárva (aszinkron irány, Editorial Rewrite marad a láncban) — a rövid távú stopgap kísérlet lezárva, negatív eredménnyel; implementáció folyamatban.
