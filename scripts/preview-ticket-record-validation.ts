...
import { resolveTabloidEvent } from "../packages/agents/src/tabloid-event";
import expandedFixtures from "../packages/agents/src/tabloid-expanded-preview-fixtures.json";
import ticketFixtures from "../packages/agents/src/tabloid-ticket-preview-fixtures.json";
import eventFixtures from "../packages/agents/src/tabloid-event-preview-fixtures.json";
...
const output = "docs/preview-ticket-record-validation.json";
...
async function main() {
  ...
  // Replay the SAME stored Preview sample first; do not refetch or change its images.
  const previous = JSON.parse(await readFile("docs/preview-rss-validation.json", "utf8"));
  const baseline = await db.execute(
    sql`SELECT id FROM raw_articles WHERE extracted_entities->'previewValidation'->>'runId'=${previous.runId} ORDER BY ingested_at,id`,
  );
  let replayed = 0;
  for (const { id } of baseline) {
    if (++replayed % 100 === 0) console.log(JSON.stringify({ baselineReplayed: replayed }));
    const raw = await rawRepo.getById(String(id));
    check(raw && !raw.storyId, "Baseline missing or linked to Story");
    const source = registry.find((s) => s.id === raw!.sourceId)!;
    check(source, "Baseline source not approved");
    ...
    const identity = accepted
      ...
      : null;
    await db.execute(
      sql`UPDATE raw_articles SET extracted_entities=coalesce(extracted_entities,'{}'::jsonb)||${JSON.stringify({ previewP0: { runId, accepted } })}::jsonb WHERE id=${raw!.id}`,
    );
    result.baselineReplay.push({
      id: raw!.id,
      source: source.name,
      title: raw!.titleOriginal,
      accepted,
  ...
  for (const event of [...new Set(eventFixtures.map((f) => f.event))]) {
    const fixtures = eventFixtures.filter((f) => f.event === event);
    const actual = [];
    for (const f of fixtures) {
      const raw = await rawRepo.getById(f.id);
      check(
        raw?.titleOriginal === f.title && raw?.bodyOriginal === f.content,
        "Mandatory Preview fixture changed",
      );
      const identity = await rawRepo.getTabloidEvent(f.id);
      check(identity, "Missing mandatory event identity");
      actual.push({
        id: f.id,
        source: f.source,
        language: f.language,
        fingerprint: identity!.fingerprint,
        canonicalRawId: identity!.canonicalRawId,
      });
    }
    const count = new Set(actual.map((r) => r.fingerprint)).size;
    result.eventGroups.push({
      event,
      sourcesFound: new Set(fixtures.map((f) => f.sourceId)).size,
      oldFingerprintCount: fixtures.length,
      newCanonicalCount: count,
      pass: count === 1,
      rows: actual,
    });
  }
  // Exact additional Preview raws; this only assigns candidate identity, never
  // eligibility, queue events, Stories or versions. REJECT remains REJECT.
  for (const group of [
    { event: "Ticket-enforcement: season ticket revocations", articles: ticketFixtures },
    ...expandedFixtures,
  ]) {
    const actual = [];
    for (const fixture of group.articles) {
      const raw = await rawRepo.getById(fixture.id);
      check(
        raw?.titleOriginal === fixture.title && raw.bodyOriginal === fixture.content,
        "Additional Preview raw changed",
      );
      const identity = await resolveTabloidEvent(
        {
          id: raw!.id,
          sourceId: raw!.sourceId,
          title: raw!.titleOriginal,
    ...
    const count = new Set(actual.map((a) => a.fingerprint)).size;
    result.eventGroups.push({
      event: group.event,
      sourcesFound: new Set(group.articles.map((a) => a.sourceId)).size,
      oldFingerprintCount: group.articles.length,
      newCanonicalCount: count,
      pass: count === 1,
      rows: actual,
    });
  }
  result.p0Regressions = [];
  for (const id of [
    "dd6ab228-5094-4ee9-84a0-de4600ad8265",
    "c4162d81-537a-45b3-8889-9d91cf92523e",
    "1e842e16-ece5-4392-856c-43838cb0d595",
  ]) {
    const raw = await rawRepo.getById(id);
    check(raw, "Regression raw missing");
    const source = registry.find((s) => s.id === raw!.sourceId)!;
    const accepted = isFootballTabloid(
      raw!.titleOriginal,
      raw!.bodyOriginal,
    ...
    result.p0Regressions.push({ id, title: raw!.titleOriginal, accepted, pass: !accepted });
    check(!accepted, "P0 regression failed; STOP before fresh RSS");
    await db.execute(
      sql`UPDATE raw_articles SET extracted_entities=coalesce(extracted_entities,'{}'::jsonb)||${JSON.stringify({ previewP0: { runId, accepted } })}::jsonb WHERE id=${id}`,
    );
    if (!result.baselineReplay.some((a: any) => a.id === id))
      result.baselineReplay.push({
        id,
        source: source.name,
        title: raw!.titleOriginal,
  ...
  await checkpoint();
  check(
    result.eventGroups.every((g: any) => g.pass),
    "Cross-source dedup failed; STOP before fresh ingest",
  );
  for (const source of registry) {
    ...
    check(await sourceRepo.getById(source.id), "Approved Preview source missing");
      ...
      await Promise.all(
        selected.slice(i, i + 4).map(async (article) => {
          ...
          const existing = await db.execute(
            sql`SELECT id FROM raw_articles WHERE source_id=${source.id} AND (source_url=${article.sourceUrl} OR extracted_entities->>'rssGuid'=${article.guid ?? article.sourceUrl}) LIMIT 1`,
          );
          const existingRaw = existing[0] ? await rawRepo.getById(String(existing[0].id)) : null;
          ...
          const imageUrl = existingRaw ? existingRaw.imageUrl : (image?.url ?? null);
          const data = {
            sourceId: source.id,
          ...
          const inserted = existingRaw ? null : await rawRepo.insertTabloid(data, false);
          const raw = existingRaw ?? inserted;
          check(raw, "Unexpected concurrent raw insertion; stop without queuing");
          const identity = accepted
            ? await resolveTabloidEvent(
                {
                  id: raw!.id,
  ...
  result.persisted =
    await db.execute(sql`SELECT r.id,r.source_id,r.source_url,r.title_original,r.image_url,r.story_id,r.extracted_entities->'previewP0' AS decision,
    (SELECT count(*)::int FROM pipeline_jobs j WHERE j.event->'payload'->>'raw_article_id'=r.id::text) AS queued
    FROM raw_articles r WHERE r.extracted_entities->'previewP0'->>'runId'=${runId} ORDER BY r.ingested_at,r.id`);
  ...
  result.verified = {
    ...
    urlsUnchanged: result.persisted.every(
      (p: any) =>
        rows.some((r) =>
          r.items.some((i: any) => i.rawId === p.id && i.imageUrl === p.image_url),
        ) || result.baselineReplay.some((r: any) => r.id === p.id && r.imageUrl === p.image_url),
    ),
  };
  result.codeHashes = {};
  for (const f of [
    "packages/agents/src/tabloid.ts",
    "packages/agents/src/tabloid-event.ts",
    "apps/web/lib/tabloid.ts",
