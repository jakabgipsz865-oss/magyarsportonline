...
/** Catches an English claim/evidence fragment copied verbatim into the Hungarian output. */
function matchesFactVerbatim(text: string, facts: WriterFact[]): boolean {
  ...
  return facts.some((fact) => {
    return [fact.claimEn, fact.evidenceOriginal, fact.detailHu].some((value) => {
      if (!value) return false;
      const factText = value.trim().toLowerCase();
 ...
 * translated into Hungarian, or text that's just a Fact's English claim or
 * evidence copied through. Deliberately
